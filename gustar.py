...
class GustarUser(db.Model):
   ...
   @staticmethod
   def get_gustar(user):
      if not user: # TODO: Prompt for new user
         return None
         
      q = db.Query(GustarUser).filter('user =', user)
      if q.count(limit=1):
         return q.get()
      else:
         gustar = GustarUser(user=user, name="Default", level=0)
         gustar.put()
         return gustar
...
class InboxPage(BaseRequestHandler):
  ...
  def get(self):
    ...
    gustar = GustarUser.get_current_gustar()
...
class GustarAction(BaseRequestHandler):
  def post(self):
    action = self.request.get('action')
    gustar = GustarUser.get_current_gustar()
    
    #~ key = self.request.get('gustar', allow_multiple=True)
    #~ gustar = Gustar.get(key)
    
    logging.info("HELLO")
    logging.info(str(gustar.user) + " " + gustar.name)
    
    if not action in ['Archive', 'Unarchive', 'Delete', 'LevelUp', 'LevelDown']:
      self.error(403)
      return

   
    #~ if gustar_key:
      #~ gustar = GustarUser.get(gustar)
      #~ if not gustar:
        #~ self.error(403)
        #~ return
      #~ else:
